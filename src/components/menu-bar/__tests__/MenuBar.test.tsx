...
import { changeSelectedLanguage, getLanguageStats } from "../../../store/menuBarSlice.ts";
...
describe("MenuBar", () => {
	...
	const clickToMenuItem = async (wrapper: ReactWrapper, searchingItem: Languages) => {
		...
		const searchedLanguageNode = wrapper
			.findWhere((node: any) => {
				return node.type() && node.name() && node.text() === searchedLanguage!.displayName;
			})
			.at(0);
		await act(async () => {
